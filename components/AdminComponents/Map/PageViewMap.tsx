...
function hexToRgba(hex: string, alpha: number): string {
  ...
  if (h.length !== 3 && h.length !== 6) return `rgba(74, 222, 128,${alpha})`;
...
export default function PageViewMap({ data, height = 380 }: PageViewMapProps) {
  ...
  useEffect(() => {
    ...
    const accent = (cs?.getPropertyValue('--admin-rail').trim() || '#4ade80');
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderLeft: '3px solid var(--admin-rail, #4ade80)',
          borderRadius: '2px',
